...
// BenchmarkStoreReplaceMemory measures allocations during Replace with stale object purging.
func BenchmarkStoreReplaceMemory(b *testing.B) {
	b.ReportAllocs()

	store := newTestStore()
	tracker := NewCardinalityTracker(100000, 0.8)
	store.SetCardinalityTracker(tracker)

	for range b.N {
		items := make([]interface{}, 100)
		for j := range 100 {
			items[j] = makeUnstructured(string(rune(j)), "pod", "default")
		}
